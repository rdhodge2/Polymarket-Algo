"""
06 - Regime Filter (YES/NO-safe, Absolute Spread) — FIXED

Fixes:
- Robust best bid/ask extraction
- Computes mid/spread even if book is crossed (flags crossed=True)
- Never formats None with :.4f (safe debug + safe returns)
- YES/NO-safe: fetches both orderbooks and picks the token with mid closest to 0.50
"""

import sys
import json
from datetime import datetime, timezone
from typing import Dict, Any, Optional, List, Tuple
from importlib import import_module

indicators = import_module("02_indicators")

# =============================
# Thresholds (Tune Here)
# =============================
MAX_BTC_ATR = 0.015
MAX_BB_WIDTH = 0.020

MAX_SPREAD_ABS = 0.12     # ABS spread in $ (0..1 contract)
MIN_MID_PRICE = 0.20
MAX_MID_PRICE = 0.80

MIN_ORDERBOOK_BALANCE = 0.40
MAX_ORDERBOOK_BALANCE = 0.60


def _fmt(x: Any, places: int = 4, default: str = "None") -> str:
    try:
        if x is None:
            return default
        return f"{float(x):.{places}f}"
    except Exception:
        return default


class RegimeFilter:
    def __init__(
        self,
        max_btc_atr: float = MAX_BTC_ATR,
        max_bb_width: float = MAX_BB_WIDTH,
        max_spread_abs: float = MAX_SPREAD_ABS,
        min_mid_price: float = MIN_MID_PRICE,
        max_mid_price: float = MAX_MID_PRICE,
        min_balance: float = MIN_ORDERBOOK_BALANCE,
        max_balance: float = MAX_ORDERBOOK_BALANCE,
        debug: bool = True,
    ):
        self.max_atr = float(max_btc_atr)
        self.max_bb_width = float(max_bb_width)
        self.max_spread_abs = float(max_spread_abs)
        self.min_mid_price = float(min_mid_price)
        self.max_mid_price = float(max_mid_price)
        self.min_balance = float(min_balance)
        self.max_balance = float(max_balance)
        self.debug = bool(debug)

        print("✅ [06] Regime filter initialized")
        print(f"   Max BTC ATR: {self.max_atr:.3f} ({self.max_atr*100:.1f}%)")
        print(f"   Max BB Width: {self.max_bb_width:.3f} ({self.max_bb_width*100:.1f}%)")
        print(f"   Max Spread (ABS): {self.max_spread_abs:.3f} (${self.max_spread_abs:.2f})")
        print(f"   Price Zone: ${self.min_mid_price:.2f}-${self.max_mid_price:.2f}")
        print(f"   Balance Zone: {self.min_balance:.0%}-{self.max_balance:.0%}")

    # =========================================================
    # Helpers
    # =========================================================
    def _parse_jsonish(self, v: Any) -> Any:
        if v is None:
            return None
        if isinstance(v, (list, dict)):
            return v
        if isinstance(v, str):
            s = v.strip()
            if (s.startswith("[") and s.endswith("]")) or (s.startswith("{") and s.endswith("}")):
                try:
                    return json.loads(s)
                except Exception:
                    return v
        return v

    def _normalize_price_0_1(self, p: Optional[float]) -> Optional[float]:
        if p is None:
            return None
        try:
            p = float(p)
        except Exception:
            return None

        if 0.0 <= p <= 1.0:
            return p
        if 1.0 < p <= 100.0:
            return p / 100.0
        return p

    def _best_bid_ask_from_book(self, book: Dict[str, Any]) -> Tuple[Optional[float], Optional[float]]:
        """
        Robust: compute best bid = max(bids prices), best ask = min(asks prices),
        regardless of ordering. Also supports book["best_bid"]/["best_ask"] if present.
        """
        bid = book.get("best_bid", None)
        ask = book.get("best_ask", None)

        bids = book.get("bids") or []
        asks = book.get("asks") or []

        def _prices(levels):
            out = []
            for lvl in levels:
                if not isinstance(lvl, dict):
                    continue
                pr = lvl.get("price")
                pr = self._normalize_price_0_1(pr)
                if pr is not None:
                    out.append(pr)
            return out

        bid_prices = _prices(bids)
        ask_prices = _prices(asks)

        # If best_bid not provided, compute max from bids
        if bid is None:
            bid = max(bid_prices) if bid_prices else None
        # If best_ask not provided, compute min from asks
        if ask is None:
            ask = min(ask_prices) if ask_prices else None

        return self._normalize_price_0_1(bid), self._normalize_price_0_1(ask)

    def _compute_spreads(self, book: Dict[str, Any]) -> Dict[str, Any]:
        """
        Always computes mid/spread when bid+ask exist.
        If crossed (ask < bid), flags crossed=True and still computes (abs diff).
        """
        bid, ask = self._best_bid_ask_from_book(book)

        if bid is None or ask is None:
            return {
                "bid": bid,
                "ask": ask,
                "mid": None,
                "spread_abs": None,
                "spread_rel": None,
                "crossed": None,
            }

        crossed = ask < bid
        spread_abs = abs(ask - bid)
        mid = (ask + bid) / 2.0
        spread_rel = (spread_abs / mid) if mid and mid > 0 else None

        return {
            "bid": bid,
            "ask": ask,
            "mid": mid,
            "spread_abs": spread_abs,
            "spread_rel": spread_rel,
            "crossed": crossed,
        }

    def _get_token_ids_and_outcomes(self, market: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        token_ids = (
            market.get("clobTokenIds")
            or market.get("clob_token_ids")
            or market.get("token_ids")
            or market.get("tokens")
        )
        token_ids = self._parse_jsonish(token_ids)

        if isinstance(token_ids, list) and token_ids and isinstance(token_ids[0], dict):
            token_ids = [t.get("token_id") or t.get("tokenId") for t in token_ids]

        token_ids = [t for t in (token_ids or []) if t]

        outcomes = self._parse_jsonish(market.get("outcomes")) or []
        if not isinstance(outcomes, list):
            outcomes = []

        return token_ids[:2], outcomes[:2]

    def _get_book_for_token(self, poly_client, token_id: str) -> Optional[Dict[str, Any]]:
        try:
            return poly_client.get_orderbook(token_id)
        except Exception as e:
            if self.debug:
                print(f"      [DEBUG] get_orderbook failed for {token_id[:20]}...: {e}")
            return None

    def _pick_tradable_outcome(self, poly_client, market: Dict[str, Any], target_mid: float = 0.50) -> Dict[str, Any]:
        token_ids, outcomes = self._get_token_ids_and_outcomes(market)
        if len(token_ids) < 2:
            return {
                "selected": None,
                "other": None,
                "market_slug": market.get("slug"),
                "market_end": market.get("endDateIso") or market.get("endDate"),
            }

        candidates = []
        for i, tid in enumerate(token_ids):
            label = outcomes[i] if i < len(outcomes) else f"Outcome{i}"
            book = self._get_book_for_token(poly_client, tid)
            sp = self._compute_spreads(book) if book else None
            mid = sp["mid"] if sp else None

            candidates.append(
                {
                    "token_id": tid,
                    "label": label,
                    "book": book,
                    "sp": sp,
                    "mid": mid,
                }
            )

        valid = [c for c in candidates if c["mid"] is not None]
        if not valid:
            return {
                "selected": candidates[0] if candidates else None,
                "other": candidates[1] if len(candidates) > 1 else None,
                "market_slug": market.get("slug"),
                "market_end": market.get("endDateIso") or market.get("endDate"),
            }

        selected = min(valid, key=lambda c: abs(c["mid"] - target_mid))
        other = next((c for c in candidates if c["token_id"] != selected["token_id"]), None)

        return {
            "selected": selected,
            "other": other,
            "market_slug": market.get("slug"),
            "market_end": market.get("endDateIso") or market.get("endDate"),
        }

    # =========================================================
    # Regime Checks
    # =========================================================
    def check_regime(self, btc_prices: List[float], orderbook: Dict[str, Any]) -> Dict[str, Any]:
        checks: Dict[str, Dict[str, Any]] = {}

        # BTC ATR
        if len(btc_prices) >= 15:
            atr = indicators.calculate_atr(btc_prices, period=15)
            checks["btc_volatility"] = {"value": atr, "threshold": self.max_atr, "pass": atr < self.max_atr, "name": "BTC Volatility (ATR)"}
        else:
            checks["btc_volatility"] = {"value": 0.0, "threshold": self.max_atr, "pass": True, "name": "BTC Volatility (ATR)", "note": "Insufficient data"}

        # BTC BB width
        if len(btc_prices) >= 20:
            bb_u, bb_l, bb_m = indicators.calculate_bollinger_bands(btc_prices, period=20)
            bb_w = (bb_u - bb_l) / bb_m if bb_m > 0 else 0.0
            checks["btc_trend"] = {"value": bb_w, "threshold": self.max_bb_width, "pass": bb_w < self.max_bb_width, "name": "BTC Trend (BB Width)"}
        else:
            checks["btc_trend"] = {"value": 0.0, "threshold": self.max_bb_width, "pass": True, "name": "BTC Trend (BB Width)", "note": "Insufficient data"}

        sp = self._compute_spreads(orderbook)
        mid = sp["mid"]

        # Price zone
        price_zone_pass = (mid is not None) and (self.min_mid_price < mid < self.max_mid_price)
        checks["price_zone"] = {
            "value": mid if mid is not None else -1,
            "min_threshold": self.min_mid_price,
            "max_threshold": self.max_mid_price,
            "pass": price_zone_pass,
            "name": "Price Zone",
            "note": None if mid is not None else "Missing bid/ask",
        }

        # Spread ABS
        spread_abs = sp["spread_abs"]
        crossed = sp.get("crossed", False)
        spread_pass = (spread_abs is not None) and (spread_abs < self.max_spread_abs) and (crossed is False)

        if self.debug:
            print(
                f"      [DEBUG] Book: bid=${_fmt(sp['bid'])} ask=${_fmt(sp['ask'])} "
                f"mid=${_fmt(mid)} spread=${_fmt(spread_abs)} crossed={crossed}"
            )
            print(
                f"      [DEBUG] SpreadCheck: spread=${_fmt(spread_abs)} vs max ${_fmt(self.max_spread_abs)} "
                f"and crossed=False → {'PASS' if spread_pass else 'FAIL'}"
            )

        checks["spread_abs"] = {
            "value": spread_abs if spread_abs is not None else 999.0,
            "threshold": self.max_spread_abs,
            "pass": spread_pass,
            "name": "Spread (Absolute)",
            "note": "Crossed book" if crossed else (None if spread_abs is not None else "Missing bid/ask"),
        }

        # Balance
        bid_depth = float(orderbook.get("bid_depth", 0) or 0)
        ask_depth = float(orderbook.get("ask_depth", 0) or 0)
        total = bid_depth + ask_depth
        if total > 0:
            bid_ratio = bid_depth / total
            balance_pass = self.min_balance < bid_ratio < self.max_balance
            checks["orderbook_balance"] = {"value": bid_ratio, "min_threshold": self.min_balance, "max_threshold": self.max_balance, "pass": balance_pass, "name": "Orderbook Balance"}
        else:
            checks["orderbook_balance"] = {"value": 0.0, "min_threshold": self.min_balance, "max_threshold": self.max_balance, "pass": False, "name": "Orderbook Balance", "note": "No depth"}

        all_pass = all(c["pass"] for c in checks.values())
        num_passed = sum(1 for c in checks.values() if c["pass"])
        regime_score = num_passed / len(checks)

        reason = None
        if not all_pass:
            failed = [c["name"] for c in checks.values() if not c["pass"]]
            reason = f"Failed: {', '.join(failed)}"

        return {"regime_ok": all_pass, "regime_score": regime_score, "checks": checks, "reason": reason, "timestamp": datetime.now(timezone.utc)}

    def check_regime_market(self, btc_prices: List[float], poly_client, market: Dict[str, Any]) -> Dict[str, Any]:
        picked = self._pick_tradable_outcome(poly_client, market, target_mid=0.50)
        selected = picked.get("selected")

        if self.debug:
            print(f"      [DEBUG] Market: {picked.get('market_slug')}")
            print(f"      [DEBUG] End: {picked.get('market_end')}")

        if not selected or not selected.get("book"):
            if self.debug:
                print("      [DEBUG] No valid orderbook → REGIME FAIL")
            dummy = {"bid_depth": 0, "ask_depth": 0}
            result = self.check_regime(btc_prices, dummy)
            result["reason"] = (result["reason"] or "") + " | No valid orderbook"
            result["selected_token"] = None
            result["market_slug"] = picked.get("market_slug")
            result["market_end"] = picked.get("market_end")
            return result

        book = selected["book"]

        # Ensure depths exist
        if "bid_depth" not in book:
            bids = book.get("bids") or []
            book["bid_depth"] = sum(float(b.get("size", 0) or 0) for b in bids[:5]) if bids else 0.0
        if "ask_depth" not in book:
            asks = book.get("asks") or []
            book["ask_depth"] = sum(float(a.get("size", 0) or 0) for a in asks[:5]) if asks else 0.0

        sp = selected.get("sp") or self._compute_spreads(book)

        if self.debug:
            print(f"      [DEBUG] Selected: {selected.get('label')} (token: {str(selected.get('token_id'))[:20]}...)")
            print(
                f"      [DEBUG] SelectedBook: bid=${_fmt(sp.get('bid'))} ask=${_fmt(sp.get('ask'))} "
                f"mid=${_fmt(sp.get('mid'))} spread=${_fmt(sp.get('spread_abs'))} crossed={sp.get('crossed')}"
            )
            other = picked.get("other")
            if other and other.get("book"):
                sp2 = other.get("sp") or self._compute_spreads(other["book"])
                print(f"      [DEBUG] Other: {other.get('label')} mid=${_fmt(sp2.get('mid'))}")

        result = self.check_regime(btc_prices, book)
        result["selected_token"] = {"label": selected.get("label"), "token_id": selected.get("token_id"), "mid": sp.get("mid")}
        result["market_slug"] = picked.get("market_slug")
        result["market_end"] = picked.get("market_end")
        return result

    def print_regime_status(self, regime_result: Dict[str, Any]) -> None:
        ok = regime_result["regime_ok"]
        score = regime_result["regime_score"]

        status_emoji = "✅" if ok else "❌"
        print(f"\n{status_emoji} Regime: {'SAFE TO TRADE' if ok else 'DO NOT TRADE'}")
        print(f"   Score: {score:.1%} ({int(score * len(regime_result['checks']))}/{len(regime_result['checks'])} passed)")
        if not ok:
            print(f"   Reason: {regime_result['reason']}")

        if regime_result.get("market_slug"):
            print(f"   Market: {regime_result.get('market_slug')}")

        st = regime_result.get("selected_token")
        if st:
            print(f"   Token: {st.get('label')} (mid=${_fmt(st.get('mid'))})")

        print("\n   Checks:")
        for _, c in regime_result["checks"].items():
            emoji = "✓" if c["pass"] else "✗"
            name = c["name"]
            if "min_threshold" in c:
                print(f"   {emoji} {name}: {_fmt(c['value'], 3)} (range: {_fmt(c['min_threshold'],2)}-{_fmt(c['max_threshold'],2)})")
            else:
                print(f"   {emoji} {name}: {_fmt(c['value'])} (max: {_fmt(c['threshold'])})")
            if c.get("note"):
                print(f"      Note: {c['note']}")
        print()


print("✅ [06] Regime filter loaded (FIXED)")


if __name__ == "__main__":
    print("\n🧪 Testing [06] - Regime Filter\n" + "=" * 90)

    try:
        alpaca_module = import_module("03_alpaca_client")
        poly_module = import_module("04_polymarket_client")
        AlpacaClient = alpaca_module.AlpacaClient
        PolymarketClient = poly_module.PolymarketClient
    except ImportError as e:
        print(f"❌ Could not import clients: {e}")
        sys.exit(1)

    filt = RegimeFilter(debug=True)
    alpaca = AlpacaClient()
    poly = PolymarketClient()

    btc_prices = alpaca.get_price_series(timeframe="1Min", limit=60) or [95000 + i * 10 for i in range(60)]

    markets = poly.get_active_btc_eth_15m_updown_markets(window_minutes=45, include_eth=True, print_markets=False)
    if not markets:
        print("⚠️  No active markets found")
        sys.exit(0)

    market = markets[0]
    print(f"✅ Testing with market: {market.get('slug')}\n")

    result = filt.check_regime_market(btc_prices, poly, market)
    filt.print_regime_status(result)

    print("=" * 90)
    print("✅ Test complete")
    print("=" * 90 + "\n")
